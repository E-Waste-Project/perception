...
yxyx_to_xywh_xyxy = {'x1': lambda x: x[1], 'w': lambda x: x[3] - x[1], 'y1': lambda x: x[0],
               'h': lambda x: x[2] - x[0], 'xc': lambda x: (x[1] + x[3]) / 2.0,
               'yc': lambda x: (x[0] + x[2]) / 2.0, 'x2': lambda x: x[3], 'y2': lambda x:x[2]}
...
class CocoDataset():
    def __init__(self, annotation_path=None, image_dir=None):
        ...
        if self.annotation_path is not None:
            ...
            self.cname2cid = {c['name']:c['id'] for c in self.coco['categories']}
    ...
    out_format=('x1', 'y1', 'w', 'h'), relative=False):
        all_imgs_labels = {}
            ...
            for c in img:
                for box in img[c]:
                    ...
                    if out_format is not None:
                        bbox[:4] = convert_format(bbox[:4], self.format, out_format)
