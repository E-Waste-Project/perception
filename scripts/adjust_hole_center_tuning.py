...
from perception.laptop_perception_helpers import detect_laptop, read_and_resize, perimeter, circularity, adjust_hole_center

data_dir = "/home/abdelrhman/data/holes/hole_"
dset_sz = 17
image_id = 0
original_img = read_and_resize(data_dir, image_id, size=None)
...
if use_canny:
    cv2.createTrackbar('thresh1', 'output_window',
                       thresh1, 1000, lambda x: None)
    cv2.createTrackbar('thresh2', 'output_window',
                       thresh2, 1000, lambda x: None)
...
while True:
    ...
    ret, output = cv2.threshold(output, thresh1, 255, cv2.THRESH_BINARY_INV)
